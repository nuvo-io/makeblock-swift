...
open class BluetoothConnection: NSObject, Connection, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    open func connectDefaultDevice() {
        ...
        let time = DispatchTime.now() + Double(5 * Int64(NSEC_PER_SEC)) / Double(NSEC_PER_SEC)
            ...
            if peripheral.state == .connected {
                if let characteristic = writeCharacteristic {
                    ...
                    while true {
                        ...
                        let dataChunk = Data(bytes: UnsafeRawPointer((data as NSData).bytes+sendIndex), count: amountToSend)
